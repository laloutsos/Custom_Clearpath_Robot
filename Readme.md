...
| ● | Driving Jackal via `cmd_vel` | This section explains how to control the Jackal robot by publishing velocity commands to the `/j100_000/cmd_vel` topic using the `geometry_msgs/msg/TwistStamped` interface, describing both the message structure (linear and angular motion components) and practical examples for sending commands from the terminal or a Python ROS2 node. | See [folder](/Custom_Jackal_J100_Tutorials/Driving_Jackal.md) for full code and execution examples. |